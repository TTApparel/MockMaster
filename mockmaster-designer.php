...
class MockMasterDesigner {
    ...
    public function render_admin_page() {
        ...
        $product = $this->get_preview_product();
        $image_url = $product ? wp_get_attachment_image_url($product->get_image_id(), 'large') : '';
        $preview_data = $this->build_product_data($product);
        ...
        <div class="wrap">
            ...
            <h2>Preview</h2>
            <?php if ($selected_product_id && !empty($preview_data['colors'])) : ?>
                <div class="notice notice-info">
                    <p><strong>Swatch debug:</strong></p>
                    <ul>
                        <?php foreach ($preview_data['colors'] as $color_key => $color_data) : ?>
                            <li>
                                <?php echo esc_html($color_data['label'] ?? $color_key); ?> —
                                <?php echo esc_url($color_data['swatch'] ?? ''); ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
